...
class LibriMixDataset(Dataset):
    ...
    def __init__(
        ...
        if task == "enh_single":
            ...
            if stage == "train-100" or stage == "train":
                md_file = "mixture_train-100_mix_clean.csv"
            elif stage == "train-360":
                md_file = "mixture_train-360_mix_clean.csv"
            elif stage == "valid":
                md_file = "mixture_dev_mix_clean.csv"
            elif stage == "test":
                md_file = "mixture_test_mix_clean.csv"
            self.csv_path = os.path.join(self.csv_dir, md_file)
        elif task == "sep_noisy":
            md_file = [f for f in os.listdir(csv_dir) if "both" in f][0]
            self.csv_path = os.path.join(self.csv_dir, md_file)
        ...
        print("Asteroid in /mnt/aoni04/saijo/asteroid", self.preprocessing)

        if num_data is not None:
            self.df = self.df[:num_data]
    ...
    def __getitem__(self, idx):
        ...
        if self.seg_len is not None and (row["length"] - self.seg_len) > 0:
            start = random.randint(0, row["length"] - self.seg_len)
            stop = start + self.seg_len
        else:
            start = 0
        ...
        sources -= sources.mean(dim=-1, keepdim=True)

        # divide by standard deviation
        if self.preprocessing == "standardization":
            std = torch.std(mixture, dim=-1, keepdim=True)
            mixture /= std
            sources /= std[..., None, :]

        if self.seg_len is not None and (row["length"] - self.seg_len) < 0:
            n_src = sources.shape[0]
            s1 = random.randint(0, self.seg_len - row["length"])
            s2 = int(self.seg_len - row["length"] - s1)
            mixture = torch.cat(
                (torch.zeros((s1,)), mixture, torch.zeros((s2,))), dim=0
            )
            sources = torch.cat(
                (torch.zeros((n_src, s1)), sources, torch.zeros((n_src, s2))),
                dim=-1,
            )
