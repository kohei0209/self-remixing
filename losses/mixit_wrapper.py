""" Copied from Asteroid toolkit and changed for our use.

References
    https://github.com/asteroid-team/asteroid/blob/master/asteroid/losses/mixit_wrapper.py
"""
...
class MixITLossWrapper(nn.Module):
    ...
    def forward(self, est_targets, targets, return_est=False, **kwargs):
        ...
        # reordered = self.reorder_source2(est_targets, targets, min_loss_idx, parts)

        # test if reordered is rightly reordered
        # reordered = reordered.sum(dim=-2)
        # n_batch = reordered.shape[0]//2
        # reordered = torch.stack((reordered[:n_batch], reordered[n_batch:]), dim=-2)
        # print(abs(reordered - reordered).sum())
    ...
    @staticmethod
    def reorder_source2(est_targets, targets, min_loss_idx, parts):
        """Reorder sources according to the best partition.
        ...
        """
        # For each batch there is a different min_loss_idx
        ordered = torch.zeros_like(est_targets)
        n_src = est_targets.shape[-2] // 2
        for b, idx in enumerate(min_loss_idx):
            right_partition = parts[idx]
            # Sum the estimated sources to get the estimated mixtures
            ordered[b, :n_src, :] = est_targets[b, right_partition[0], :]
            ordered[b, n_src:, :] = est_targets[b, right_partition[1], :]

        ordered = torch.cat((ordered[..., :n_src, :], ordered[..., n_src:, :]), dim=0)
